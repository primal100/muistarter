...
describe("test verify email view", () => {
  it("should verify email, redirect to home and show success message", async () => {
    await Promise.all([waitForNavigation(), loadVerifyEmailUrl("user_id=1&timestamp=1589819019&signature=1235&email=a@c.com")])
  ...
  it("should fail to verify email with wrong signature", async () => {
    await Promise.all([waitForNavigation(), loadVerifyEmailUrl("user_id=1&timestamp=1589819019&signature=1234&email=a@c.com")])
    expect(await url()).toBe(URL + "/");
